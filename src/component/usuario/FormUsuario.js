...
import {
    Alert, Box, Button, FormControl, InputLabel, MenuItem, Paper, Select, TextField, Grid
} from "@mui/material";
import {
...
const FormUsuario = () => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (!usuario.nombre || !usuario.apellido || !usuario.correo || !usuario.password || !usuario.password_repeat) {
            ...
            <Grid item xs={6}>
                <Paper elevation={3} style={{borderRadius: 16}}>
                    <form onSubmit={handleSubmit} style={{paddingTop: 15, paddingBottom: 15}}>
                        ...
                        {message !== null && <Box sx={{marginTop: 3, paddingLeft: 10, paddingRight: 10}}>
                        ...
                        <Box sx={boxStyle}>
                            <Edit sx={{color: 'action.active', mr: 1, my: 0.5}}/>
                            <TextField autoComplete="off" color="success" fullWidth label="Usuario (correo electrónico)"
                                       name="email"
                                       variant="standard"
                        ...
                        <Box sx={boxStyle}>
                            ...
                            <TextField autoComplete="off" color="success" fullWidth label="Contraseña" name="password"
                                       type={!viewPassword ? "password" : "text"}
                                       variant="standard" value={usuario.password}
                                       onChange={(e) => {
                                           if (e.target.value.length < 10) {
                                               setMessage({
                                                   texto: "La contraseña debe tener 10 caracteres.", type: "warning"
                                               })
                                           } else {
